...
const GLOBAL_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .nav-btn.active { color: #fff !important; border-bottom: 2px solid #fff !important; }
...
═══════════════════════════════════════════════════ */
const TABS = [
  { key:"overview",    label:"Marché",       icon:"📈" },
  { key:"morocco",     label:"Actions",      icon:"🇲🇦" },
  { key:"currencies",  label:"Forex",        icon:"💱" },
  { key:"indices",     label:"Indices",      icon:"🌍" },
  { key:"crypto",      label:"Crypto",       icon:"₿"  },
  { key:"commodities", label:"Matières",     icon:"🛢"  },
...
function Navbar({ activeTab, onTab, updatedAt }) {
  ...
  return (
    <nav style={{
      ...
      borderBottom:`1px solid ${C.border}`,
      height:52,
      position:"sticky", top:32, zIndex:500,
    }}>
      {/* Logo */}
        ...
        {TABS.map(tab => (
          <button key={tab.key} onClick={()=>onTab(tab.key)}
            ...
            style={{
              ...
              color: activeTab===tab.key ? C.text : C.text2,
              background:"none", border:"none",
              borderBottom: activeTab===tab.key ? `2px solid ${C.text}` : "2px solid transparent",
